  ...
  const fetchDetails = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(err.detail || 'Не удалось получить данные с виртуальной машины по SSH.');
      }
    ...
    return (
      <div className="console-modal-backdrop">
        <div className="console-container" style={{ maxWidth: '600px', padding: '40px', textAlign: 'center' }}>
          ...
          <p style={{ fontWeight: 600 }}>Подключение к ВМ по SSH и сбор метрик...</p>
          <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginTop: '8px' }}>
            Это может занять несколько секунд... Убедитесь, что ВМ запущена и к ней настроен SSH.
          </p>
        </div>
      </div>
    );
    ...
    return (
      <div className="console-modal-backdrop">
        <div className="console-container" style={{ maxWidth: '500px', padding: '40px', textAlign: 'center', color: 'var(--danger)' }}>
          ...
          <h3>Не удалось подключиться к ВМ по SSH</h3>
      ...
      <div className="console-container" style={{ width: '95vw', maxWidth: '1000px', height: '85vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <div className="console-header">
          <div className="console-title">
            ...
            <span>Мониторинг ВМ: <strong style={{ color: 'var(--primary)' }}>{data.name}</strong> ({data.host})</span>
          </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                {!data.docker.installed ? (
                  <div style={{
                    ...
                    Служба Docker не установлена на этой ВМ.
                  </div>
                  ...
                  <div style={{
                    ...
                    Нет запущенных Docker-контейнеров на этой ВМ.
                  </div>
                ) : (
                  <div className="card" style={{ padding: '0', overflowX: 'auto', maxHeight: '350px' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.8rem' }}>
