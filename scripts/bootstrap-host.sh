...
# (172.20.0.30-229, см. compute_static_ip в backend/app/api/vms.py). Раньше
# диапазон был 172.20.0.10-250 — он полностью накрывал статический пул, и
# любое устройство, попросившее адрес по DHCP на этом мосту, могло получить
# IP, который уже статически прописан в cloud-init другой ВМ: коллизия на
# одном L2-сегменте (ARP-конфликт), из-за которой обе машины становятся
# ненадёжно доступны. Сам dnsmasq здесь — подстраховка для того, что не
# настраивает статику самостоятельно; ВМ панели её не используют.
