...
def test_lvm_configured_but_unreachable_falls_back_to_host_disk(monkeypatch):
    """STORAGE_CLASS указывает на LVM, но vg-aegis не отвечает (ещё не
    создана, vgs недоступен) — лучше проверить хоть что-то, чем ничего."""
...
def test_reservations_with_k8s_client_include_backups(monkeypatch):
    monkeypatch.setattr(cap, "backups_total_gb", lambda k8s: 15.0)
    db = FakeDb(vms=[FakeVM(1, 1, 20)])
