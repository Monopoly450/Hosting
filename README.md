...
2. Склонируйте репозиторий и запустите установщик от имени root:
   ...
   sudo ./install.sh
   ```
   Скрипт спросит пароли для базы данных, очереди, MinIO и токен администратора —
   на каждый вопрос можно просто нажать Enter, тогда будет подставлено надёжное
   случайное значение. В конце будет напечатан адрес панели и пароль входа.
