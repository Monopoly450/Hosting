...
| Домены панели/почты/хранилища (`PANEL_DOMAIN`/`MAIL_DOMAIN`/`STORAGE_DOMAIN`) и токен Timeweb | `.env` — не в БД, читаются напрямую из окружения |
...
привязать и к трём служебным сервисам этого хоста — панели управления, вебмейлу
(Roundcube) и консоли объектного хранилища (MinIO). В обоих случаях это делает один
и тот же Caddy-контейнер (`aegis-caddy`), а домен открывается без порта в адресе:
`https://ваш-домен` вместо `https://IP:8443`.
...
DNS-провайдера, и подключение к серверу снаружи не требуется вовсе. Сейчас
поддержан один провайдер — **Timeweb Cloud** (модуль `caddy-dns/timeweb`, образ
Caddy собирается локально через `xcaddy`, см. `aegis-caddy/Dockerfile`).

Нужен API-токен Timeweb Cloud (Настройки → API-ключи) с правом на управление DNS.
DNS-01 автоматически включается для **любого** домена (панель, почта, хранилище,
клиентские ВМ), если хост определён как приватный (`is_private_host_ip`) и токен
задан — руками переключать ничего не нужно.

### Как привязать домены

**При установке** — `install.sh` сам спрашивает домены для панели/почты/хранилища
и токен Timeweb (весь блок можно пропустить, Enter на каждом вопросе — тогда всё
как обычно доступно по IP и портам).
...
этот скрипт трогает только четыре строки:
`PANEL_DOMAIN` / `MAIL_DOMAIN` / `STORAGE_DOMAIN` / `TIMEWEB_DNS_API_TOKEN`.
...
# TIMEWEB_DNS_API_TOKEN=
```
Все четыре необязательны и независимы друг от друга — можно задать только один
домен, только токен без доменов ни на что не влияет.
...
Если после нескольких неудачных попыток подряд в логе появляется
`acme-staging-v02.api.letsencrypt.org` — это защитный механизм Caddy/certmagic
от исчерпания лимитов боевого Let's Encrypt при частых сбоях; сертификат со
staging браузеры не считают доверенным. Он не переключается обратно
автоматически в рамках одного запуска — после того как DNS-запись точно
разошлась (проверить: `dig @1.1.1.1 TXT _acme-challenge.<домен> +short`),
пересоздайте контейнер один раз, чтобы форсировать повторную попытку на боевом CA:
```bash
docker rm -f aegis-caddy
```
Вотчдог в воркере поднимет его заново в течение минуты — образ уже собран,
...
```
Если и это не помогло — сначала очистите зависшие loop-устройства:
```bash
...
Создайте второго пользователя, проект, добавьте его **наблюдателем**, привяжите ВМ к проекту, войдите под вторым пользователем:
